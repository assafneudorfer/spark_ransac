...
def ransac_with_spark(csv_path, iterations, cutoff_dist):
    conf = SparkConf().setAppName("RANSAC").setMaster("local[*]")
    sc = SparkContext.getOrCreate(conf)
    spark = SparkSession(sc)
    # Read the CSV file directly from disk into a DataFrame
    ...
    # Perform an inner join based on the unique identifiers
    pairs_df = sampled_df1.join(
        sampled_df2,
        on="id",
        how="inner"
    ).drop("id")
    pairs_df = pairs_df.withColumn('dx',
                                   F.when(col("x1") - col("x2") != 0, col("x1") - col("x2")).otherwise(0.0001))
    ...
    df = df.withColumn("score", F.when(F.abs(col("y") - col("pred_y")) < cutoff_dist, F.abs(col("y") - col("pred_y"))).otherwise(cutoff_dist))
    total_score_df = df.groupBy("a", "b").agg(F.sum("score").alias("total_score"))
    min_raw = total_score_df.select(F.min(
        F.struct("total_score", *(x for x in total_score_df.columns if x != "total_score"))
    )).first()[0]
...
if __name__ == '__main__':
    ...
    best_model = ransac_with_spark(path_to_samples_csv, iterations=5000, cutoff_dist=20)
